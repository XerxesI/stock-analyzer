...
def test_ambiguous_target_exit_excludes_exit_session_from_mfe():
    """AAA's exit was an intraday-high touch (Jan 8 open=10.6 < target=11.0, but
    high=11.2 >= 11.0) -- Section 20's ambiguity rule must exclude Jan 8 from the
    MFE/MAE full-path window, so the reported MFE should come from Jan 7's High
    (10.8), not Jan 8's higher 11.2."""

    context, calendar, effective_entry_price, _ = _build_fixture()
    summary = compute_run_summary(context, REPLAY_ID, "B", None, calendar)

    # window is Jan6..Jan7 only (Jan8 excluded); High max = 10.8 (Jan 7).
    expected_mfe_pct = (10.8 - effective_entry_price) / effective_entry_price
...
def _summary_with(realized_return_pct: float | None) -> RunQualitySummary:
    ...
    buy = BuyQualitySummary(
        ...
        horizon_mean_sessions_to_mae={}, horizon_censored_count={},
    )
    hold = HoldQualitySummary(
        ...
        horizon_target_reached_rate={}, horizon_censored_count={}, by_holding_age_bucket=(), by_unrealized_return_bucket=(),
    )
    sell = SellQualitySummary(
        ...
        horizon_target_reached_rate={}, horizon_censored_count={}, total_censored_post_exit_observations=0,
    )
    capacity = CapacityQualitySummary(
        ...
        horizon_censored_count={}, mean_open_position_count=None, mean_reserved_order_count=None,
