"""Tests for EXP-005's four new tables (Revision 5, Stage 2 --
docs/09_experiments/EXP-005_Portfolio_Policy_Feasibility_Pilot.md Sections 8.1, 8.5,
18; docs/09_experiments/EXP-005_Implementation_Checklist.md).
"""
...
def _insert_admission(
    conn: sqlite3.Connection, admission_id: str, candidate_id: str, *, decision: str = "ACCEPTED", slot_budget=10_000.0
) -> None:
    conn.execute(
        ...
        (admission_id, "replay-1", candidate_id, "AAA", "2026-01-05", decision, 1, slot_budget, None, NOW),
    )
...
def test_reservation_rejects_nonexistent_admission(conn: sqlite3.Connection):
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            ...
            ("r1", "replay-1", "no-such-admission", "AAA", "AAA", 10_000.0, "RESERVED", NOW, None),
        )
    ...
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            ...
            (
                ...
                "2026-01-05", "2026-01-06", 100.0, 100.05, 10.0, 1000.0, 1.0, 0.0005, 0.5, 999.0,
...
def test_duplicate_reservation_for_one_admission_is_rejected(conn: sqlite3.Connection):
    ...
    conn.execute(
        "INSERT INTO slot_reservations (reservation_id, replay_id, admission_id, candidate_id, symbol, "
        " reserved_amount, status, created_at, resolved_at) VALUES (?,?,?,?,?,?,?,?,?)",
        ("r1", "replay-1", "c1", "c1", "AAA", 10_000.0, "RESERVED", NOW, None),
    )
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            "INSERT INTO slot_reservations (reservation_id, replay_id, admission_id, candidate_id, symbol, "
            " reserved_amount, status, created_at, resolved_at) VALUES (?,?,?,?,?,?,?,?,?)",
            ("r2-different-id", "replay-1", "c1", "c1", "AAA", 10_000.0, "RESERVED", NOW, None),
        )
...
def test_admission_decision_cannot_be_null(conn: sqlite3.Connection):
    ...
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            "INSERT INTO portfolio_admissions (admission_id, replay_id, candidate_id, symbol, as_of_date, "
            " decision, rank_at_admission, slot_budget, reason, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
            ("c1", "replay-1", "c1", "AAA", "2026-01-05", None, 1, 10_000.0, None, NOW),
        )
...
def test_admission_decision_slot_budget_consistency_check(conn: sqlite3.Connection):
    ...
    _insert_candidate(conn, "c1")
    with pytest.raises(sqlite3.IntegrityError):
        _insert_admission(conn, "c1", "c1", decision="ACCEPTED", slot_budget=None)
    _insert_candidate(conn, "c2", symbol="BBB")
    with pytest.raises(sqlite3.IntegrityError):
        _insert_admission(conn, "c2", "c2", decision="NO_CAPACITY", slot_budget=10_000.0)
...
def test_reservation_status_resolved_at_consistency_check(conn: sqlite3.Connection):
    ...
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            "INSERT INTO slot_reservations (reservation_id, replay_id, admission_id, candidate_id, symbol, "
            " reserved_amount, status, created_at, resolved_at) VALUES (?,?,?,?,?,?,?,?,?)",
            ("r1", "replay-1", "c1", "c1", "AAA", 10_000.0, "RESERVED", NOW, NOW),  # RESERVED + resolved_at set
        )
...
def test_execution_quantity_must_be_positive(conn: sqlite3.Connection):
    ...
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            ...
            (
                "e1", "replay-1", "B", None, None, "c1", None, "AAA", "SELL",
                "2026-01-05", "2026-01-06", 100.0, 100.05, 0.0, 1000.0, 1.0, 0.0005, 0.5, 999.0,
                "SELL_TARGET", "snap-1", NOW,
            ),
        )
    ...
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            ...
            (
                ...
                "2026-01-05", "2026-01-06", 100.0, 100.05, 10.0, 1000.0, 1.0, 0.0005, 0.5, -1001.5,
...
def test_equity_snapshot_rejects_second_row_for_same_day(conn: sqlite3.Connection):
    conn.execute(
        "INSERT INTO portfolio_equity_snapshots (snapshot_id, replay_id, as_of_date, cash, reserved_capital, "
        " open_position_market_value, total_equity, open_position_count, reserved_order_count, "
        " cumulative_commissions, cumulative_slippage_cost, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
        ("s1", "replay-1", "2026-01-05", 100_000.0, 0.0, 0.0, 100_000.0, 0, 0, 0.0, 0.0, NOW),
    )
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            "INSERT INTO portfolio_equity_snapshots (snapshot_id, replay_id, as_of_date, cash, reserved_capital, "
            " open_position_market_value, total_equity, open_position_count, reserved_order_count, "
            " cumulative_commissions, cumulative_slippage_cost, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
            ("s2-different-id", "replay-1", "2026-01-05", 99_000.0, 0.0, 0.0, 99_000.0, 0, 0, 0.0, 0.0, NOW),
        )
...
def test_deterministic_ordering_keys_exist_on_every_new_table(conn: sqlite3.Connection):
    # Every table has at least one column suitable for an explicit ORDER BY beyond
    # its primary key -- as_of_date/execution_date/created_at -- so Stage 3's
    # repository queries never need to depend on SQLite's unspecified row order.
    for table, column in (
